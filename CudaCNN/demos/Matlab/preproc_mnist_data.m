function [inp,targ] = preproc_mnist_data(id,n,labels,rand_on)
%preproc_data MNIST handwriten image database preprocessing.
%
%  Syntax
%  
%    [pd,labnew] = preproc_mnist_data(id,n,labels,rand_on)
%    
%  Description
%   Input:
...
for k=1:n
    ...
    d = -ones(1,10);
    d(labels(rand_num)+1) = 1;
    targ{k} = d;
    randd{k} = zeros(32,32);
    randd{k}(3:30,3:30)=double(id{rand_num});
    inp{k} = reshape(mapstd(reshape(randd{k},1,[])),32,32);
end
