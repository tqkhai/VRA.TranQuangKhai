function save_cnet_to_hdf5(cnet_obj, filename)
%Convert to struct
...
function write_slayer(filename, slayer, lnum)
    ...
    hdf5writeAttribute(filename, ['/Layers/Layer' num2str(lnum)], 'InpWidth', uint32(slayer.FMapWidth*slayer.SRate));
    hdf5writeAttribute(filename, ['/Layers/Layer' num2str(lnum)], 'InpHeight', uint32(slayer.FMapHeight*slayer.SRate));
...
function write_clayer(filename, clayer, lnum)
    ...
    hdf5writeAttribute(filename, ['/Layers/Layer' num2str(lnum)], 'TransferFunc', ...
        uint32(get_trasfer_func_enum(clayer.TransfFunc)));    
    ...
    %TODO: change OutFMapWidth to InpWidth
